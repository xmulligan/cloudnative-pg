...
- Restricted subscription parameter updates to only valid PostgreSQL options,
  preventing unsupported or unsafe changes
...
- Restored the correct initialization order in the `initdb` job, ensuring
  consistent cluster bootstrapping
  ([#7871](https://github.com/cloudnative-pg/cloudnative-pg/pull/7871)).
